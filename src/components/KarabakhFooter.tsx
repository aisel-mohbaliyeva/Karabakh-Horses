...
const links = {
  Explore: ["About", "Gallery", "Facts", "Horse's"],
  Connect: ["Instagram", "YouTube", "WhatsApp", "Wikipedia"],
};
    ...
    <footer className="relative" style={{ padding: "var(--section-pad) 0 0", background: "hsl(var(--foreground))" }}>
      ...
      <div className="container">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1.5fr] gap-[clamp(24px,4vw,60px)] pb-[clamp(40px,6vw,70px)]">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <p className="text-sm leading-7 mb-5 max-w-[300px]" style={{ color: "hsla(var(--cream) / 0.45)" }}>
              Preserving the golden legacy of Azerbaijan's most treasured breed — one story, one stride at a time.
            </p>
            <a href="https://en.wikipedia.org/wiki/Karabakh_horse" target="_blank" rel="noreferrer" className="text-xs font-semibold tracking-[1.5px] uppercase transition-all hover:tracking-[2px]" style={{ color: "hsl(var(--gold))" }}>Learn more on Wikipedia →</a>
          </div>

          {Object.entries(links).map(([heading, items]) => (
            <div key={heading}>
              <h4 className="text-[11px] font-bold tracking-[3px] uppercase mb-4" style={{ color: "hsl(var(--gold))" }}>{heading}</h4>
              <ul className="flex flex-col gap-2.5 list-none">
                {items.map(item => (
                  <li key={item}>
                    <a href={`#${item.toLowerCase()}`} className="text-sm transition-all hover:pl-1.5" style={{ color: "hsla(var(--cream) / 0.45)" }}>{item}</a>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div>
            <h4 className="text-[11px] font-bold tracking-[3px] uppercase mb-4" style={{ color: "hsl(var(--gold))" }}>Stay Updated</h4>
            <p className="text-[13px] leading-relaxed mb-4" style={{ color: "hsla(var(--cream) / 0.45)" }}>Subscribe for news about breed preservation.</p>
            <div className="flex rounded-lg overflow-hidden" style={{ border: "1px solid hsla(var(--gold) / 0.25)" }}>
              <input type="email" placeholder="your@email.com" aria-label="Email" className="flex-1 min-w-0 border-none outline-none px-3.5 py-2.5 text-[13px]" style={{ background: "hsla(var(--cream) / 0.05)", color: "hsl(var(--cream))", fontFamily: "var(--font-body)" }} />
        ...
        <div className="flex items-center justify-between py-5 flex-wrap gap-2.5 max-sm:justify-center max-sm:text-center">
          <p className="text-xs tracking-[0.5px]" style={{ color: "hsla(var(--cream) / 0.3)" }}>© {new Date().getFullYear()} Karabakh Horse's. All rights reserved.</p>
